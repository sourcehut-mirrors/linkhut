defmodule LinkhutWeb.Plugs.PromEx do
  ...
  * `:path` - The path through which your metrics can be accessed (default is "/metrics")
  ...
  def call(%Plug.Conn{request_path: metrics_path} = conn, %{metrics_path: metrics_path} = opts) do
    username =
      Linkhut.Config.prometheus(:username) ||
        raise "missing config :linkhut, Linkhut.Prometheus, :username"

    password =
      Linkhut.Config.prometheus(:password) ||
        raise "missing config :linkhut, Linkhut.Prometheus, :password"
