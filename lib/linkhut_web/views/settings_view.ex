defmodule LinkhutWeb.SettingsView do
  ...
  def bookmarklet(conn) do
    ...
    get_notes_js = "(document.querySelector('meta[name=\"description\"]')!=null?document.querySelector('meta[name=\"description\"]').content:%22%22)"
    get_tags_js = "(document.querySelector('meta[name=\"keywords\"]')!=null?document.querySelector('meta[name=\"keywords\"]').content:%22%22)"
  ...
  def nav_link(conn, text, opts) do
    ...
    class = if active? do "active" else "" end
    opts = Keyword.put(opts, :class, class)
    link = make_link(active?, text, opts)
    cond do
      tag = opts[:wrap_with] -> content_tag(tag, link, class: class)
      true                   -> link
    end
  end
